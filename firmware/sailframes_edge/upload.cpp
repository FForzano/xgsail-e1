...
// trigger) but never the recording gate. Reboots on a successful update.
static void runOtaCheck(bool manual) {
...
// BLE trigger uses runOtaCheck(true) directly (see the upload task loop).
...
void uploadTaskFunc(void* param) {
  ...
  while (true) {
    ...
    // Manual OTA trigger (ble_relay.cpp control `ota-update`): bring WiFi up
    // and run a check/apply now, regardless of the ota_auto_update opt-in.
    // Refused while recording. runOtaCheck() reboots on a successful update.
    if (otaManualRequested) {
      otaManualRequested = false;
      if (logging) {
        otaSetState("error", "recording in progress");
      } else if (config.wifi_count == 0) {
        otaSetState("error", "no wifi configured");
      } else {
        wifiBusy = true;
        if (!wifiConnected) { g_uploadSection = "ota-manual.wifi-connect"; connectWiFi(); }
        if (wifiConnected) {
          g_uploadSection = "ota-manual.run";
          runOtaCheck(true);            // reboots on success
          wifiTeardownRequested = true; // release the radio on Core 1
        } else {
          otaSetState("error", "wifi connect failed");
        }
        wifiBusy = false;
      }
      continue;  // don't fall through to an upload trigger this iteration
    }
          ...
          if (gps.valid && gps.speed_kts >= 0.5) {
            ...
            if (now - stationaryStart >= 30000 &&
              ...
              if (wifiConnected) {
                ...
                // Auto OTA check while the radio is already up (no-op unless
                // opted in / a newer build exists; reboots on a real update).
                g_uploadSection = "ota-check.health-only";
        ...
        if (!isClaimed()) {
          ...
          if (!connOK && uploadRetryCount < 2) {
            ...
            if (remaining <= 0) {
              ...
              // OTA update check — piggybacks the same once-per-boot WiFi
              // window (docs/ota.md, checkForFirmwareUpdate()).
