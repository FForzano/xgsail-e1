...
void uploadTaskFunc(void* param) {
  ...
  while (true) {
    ...
    // runOtaCheck(true) does its own logging/wifiConnected gating and
    // reports state via the BLE `status` ota object (ota.cpp) — nothing
    // more to check here. Reboots on a successful update.
    if (otaManualRequested) {
      ...
      runOtaCheck(true);
      wifiTeardownRequested = true;  // release the radio on Core 1 either way
      wifiBusy = false;
